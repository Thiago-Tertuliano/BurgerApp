      ...
      <div class="ingredients-grid">
        ...
        <div class="custom-section">
          ...
          <select v-model="selected.bread" class="custom-select">
            <option v-for="option in options.bread" :key="option.id" :value="option">
        ...
        <div class="custom-section">
          ...
          <select v-model="selected.meat" class="custom-select">
            <option v-for="option in options.meat" :key="option.id" :value="option">
              {{ option.name }} (+R$ {{ option.price.toFixed(2) }})
            </option>
          </select>
        </div>
        ...
        <div class="custom-section">
          ...
          <select v-model="selected.cheese" class="custom-select">
            <option v-for="option in options.cheese" :key="option.id" :value="option">
              {{ option.name }} (+R$ {{ option.price.toFixed(2) }})
            </option>
          </select>
        </div>
        ...
        <div class="custom-section">
          ...
          <select v-model="selected.sauce" class="custom-select">
            <option v-for="option in options.sauce" :key="option.id" :value="option">
              {{ option.name }} (+R$ {{ option.price.toFixed(2) }})
            </option>
          </select>
        </div>
...
onMounted(async () => {
  try {
    ...
    options.value.bread = ingredients.value.filter(i => i.category === 'pao' || i.category === 'pão')
    options.value.meat = ingredients.value.filter(i => i.category === 'carne')
    options.value.cheese = ingredients.value.filter(i => i.category === 'queijo')
    options.value.sauce = ingredients.value.filter(i => i.category === 'molho' || i.category === 'molhos')
    
    // Selecionar primeiros itens por padrão
    if (options.value.bread.length > 0) selected.value.bread = options.value.bread[0]
    if (options.value.meat.length > 0) selected.value.meat = options.value.meat[0]
    if (options.value.cheese.length > 0) selected.value.cheese = options.value.cheese[0]
    if (options.value.sauce.length > 0) selected.value.sauce = options.value.sauce[0]
  } catch (error) {
